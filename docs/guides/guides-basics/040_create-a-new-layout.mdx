...
To define a different layout for a page, set the `default_layouts` attribute in `app/config/config.js`.
...
When loading a page, you are able to override the layout as well by passing the `layout_id` option to the `SipaPage.load` method.
