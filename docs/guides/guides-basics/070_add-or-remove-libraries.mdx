...
Let's assume we have a library called `foobar` which consists of two files, `code.js` and `style.css`.
...
Unzip the bootstrap archive in your project at `app/assets/lib/bootstrap5`, so the file structure looks like this:
